        ...
        tab.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                if (tab.getPosition() == 0) {  //开仓
                    ...
                    if (MyApplication.getApp().isLogin()) {
                        Zhang.setText(avaOpenBuy() + " 张");
                        Zhang2.setText(avaOpenSell() + " 张");
                        if (check == 0) {
                            zhangs = avaOpenBuy();
                        } else {
                            zhangs = avaOpenSell();
                    ...
                    if (MyApplication.getApp().isLogin()) {
                        Zhang2.setText((int) data.getUsdtBuyPosition() + " 张");
                        Zhang.setText((int) data.getUsdtSellPosition() + " 张");
                        if (check == 0) {
...
//                mPresenter.getOrderHistory(SharedPreferenceInstance.getInstance().getTOKEN(), startPage, pageSize, this.currency.getSymbol(), "", "", "", "");
        ...
        if (currency != null) {
            ...
            tvTitle.setText(this.currency.getSymbol() + " 永续");
...
//            mPanJia.setText(String.valueOf(currency.getClose()));
    ...
    private void refreshgranary(Double close) {
        ...
        if (data.getUsdtBuyPosition() > 0 || data.getUsdtFrozenBuyPosition() > 0) {
            ...
            //杠杆倍数
            buyOne.setText("多头 " + (int) data.getUsdtBuyLeverage() + "x");
            //持仓量
            buyTwo.setText((int) (data.getUsdtBuyPosition() + data.getUsdtFrozenBuyPosition()) + "张");
            //可平量
            buyThree.setText((int) data.getUsdtBuyPosition() + "张");
            //开仓均价
        ...
        if (data.getUsdtSellPosition() > 0 || data.getUsdtFrozenSellPosition() > 0) {
            ...
            //杠杆倍数
            sellOne.setText("空头 " + (int) data.getUsdtSellLeverage() + "x");
            //持仓量
            sellTwo.setText((int) (data.getUsdtSellPosition() + data.getUsdtFrozenSellPosition()) + "张");
            //可平量
            sellThree.setText((int) data.getUsdtSellPosition() + "张");
            //开仓均价
    ...
    public void getdetail(Detail detail) {
        if (detail.getCode() == 0) {
            ...
            if (detail.getData().getUsdtPattern().equals("FIXED")) {
                targetPattern = "1";
                tvModel.setText("逐仓模式");
            } else {
                targetPattern = "0";
                tvModel.setText("全仓模式");
            }
            ...
            if (mtabtype == 0) {
                ...
                if (check == 0) {
                    Zhang2.setText((int) data.getUsdtSellPosition() + " 张");
                    zhangs = (int) data.getUsdtSellPosition();
                } else {
                    Zhang.setText((int) data.getUsdtBuyPosition() + " 张");
                    zhangs = (int) data.getUsdtBuyPosition();
                }
            }
            refreshgranary(currency.getClose());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_limit_entrust:
                //点击限价委托
                tvEntrust.setText("限价委托");
                mOnePriceEdit.setHint("委托价");
                mTwoPriceEdit.setHint("委托价");
                type = "LIMIT_PRICE";
            ...
            case R.id.ll_plan_entrust:
                tvEntrust.setText("计划委托");
                mOnePriceEdit.setHint("默认委托价为市价");
                mOnePriceEdit.setText("");
                mTwoPriceEdit.setHint("默认委托价为市价");
        ...
        switch (type) {
            case "LIMIT_PRICE":
                ...
                if (WonderfulStringUtils.isEmpty(price, amout)) {
                    WonderfulToastUtils.showToast("请输入委托价和数量");
                    return;
                }
                if (amout.contains("%")) {
//                    zhangs = 2;
//                    if (zhangs == 0) {
//                        Toast.makeText(getActivity(), "下单数量必须大于等于1张", Toast.LENGTH_SHORT).show();
//                        return;
//                    }
                    String substring = amout.substring(0, amout.length() - 1);
                    if (substring.equals("0")) {
                        WonderfulToastUtils.showToast("下单数量必须大于0");
                        return;
                    }
                ...
                if (new BigDecimal(price).compareTo(BigDecimal.ZERO) <= 0) {
                    WonderfulToastUtils.showToast("请输入正确的委托价");
                    return;
                }
            ...
            case "MARKET_PRICE":
                ...
                amout = mOneTcpEdit.getText().toString();
                if (WonderfulStringUtils.isEmpty(price, amout)) {
                    WonderfulToastUtils.showToast("请输入数量");
                    return;
                }
                if (amout.contains("%")) {
//                    zhangs = 2;
    ...
    public void onGetMessage(SocketResponse response) {
        ...
        switch (response.getCmd()) {
            ...
            case CONTRACT_PUSH_EXCHANGE_ORDER_TRADE:
                ...
                break;
//            case CONTRACT_PUSH_SYMBOL_THUMB:
